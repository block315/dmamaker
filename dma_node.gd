...
func _process(_delta: float) -> void:
	if mouse_selected:
		...
		if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT) \
			...
			if STLIO.Importer.LoadFromPath('res://library/3dmodels/{0}.stl'.format([name.rsplit(ProgramConfig.index_split_symbol)[0]])) is Error:
				print(name.rsplit(ProgramConfig.index_split_symbol)[0] + " model doesn't exist : " + 'res://library/3dmodels/{0}.stl'.format([name.rsplit(ProgramConfig.index_split_symbol)[0]]))
				_3d_representation.mesh = STLIO.Importer.LoadFromPath('res://library/3dmodels/motormeshfortest.stl')
				_3d_representation.scale = Vector3(.001,.001,.001)
			else:
				_3d_representation.mesh = STLIO.Importer.LoadFromPath('res://library/3dmodels/{0}.stl'.format([name.rsplit(ProgramConfig.index_split_symbol)[0]]))
			var _material = StandardMaterial3D.new()
			randomize()
			_material.albedo_color = Color(randf(), randf(), randf())
			_3d_representation.mesh.surface_set_material(0,_material)
