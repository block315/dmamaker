...
func make_tree(_enabled_set:Array[String]=[""]):
	...
	if enabled_set.size() > 1:
		...
		_root.set_text(0, "Multiple DMAsets")
	
	for _set_file in enabled_set:
		var parser: XMLParser = make_parser_from_file(_set_file)
		var _tree_stack = []
		while parser.read() != ERR_FILE_EOF:
			if parser.get_node_type() == XMLParser.NODE_ELEMENT:
				if parser.get_node_name() == "mech":
					mech_pics[parser.get_named_attribute_value("name")] = parser.get_named_attribute_value("pic")
...
#rand_from_seed(connection_type)[0]/1000000%256,rand_from_seed(connection_type)[0]/1000%256)
			...
			_node.set_slot(connection_index, false, 0, Color("White"), \
				true, 0, Color("White"))
			connection_index += 1
...
func make_tree_item(_tree_stack, parser) -> TreeItem:
	...
	var _tree_item_button : ImageTexture = ImageTexture.new()
	_tree_item_button.set_image(Image.load_from_file("res://arts/kenney_game-icons/PNG/White/1x/plus.png"))
